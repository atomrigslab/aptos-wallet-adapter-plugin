...
export const TWalletName = "TWallet" as WalletName<"TWallet">;

export class TWallet implements AdapterPlugin {
  readonly name = TWalletName;
  readonly url = "https://www.twallet.ai";
  readonly icon =
  ...
  async connect(): Promise<AccountInfo> {
    try {
      ...
      if (!accountInfo) throw `${TWalletName} Address Info Error`;
  ...
  async account(): Promise<AccountInfo> {
    ...
    if (!response) throw `${TWalletName} Account Error`;
    return response;
  }
  ...
  async signMessage(message: SignMessagePayload): Promise<SignMessageResponse> {
    try {
      if (typeof message !== "object" || !message.nonce) {
        `${TWalletName} Invalid signMessage Payload`;
      }
      const response = await this.provider?.request({
      ...
      if (response) {
        ...
        throw `${TWalletName} Sign Message failed`;
      }
    } catch (error: any) {
      const errMsg = error.message;
      throw errMsg;
